...
run_step "Step 3 targeting-dynamic" \
  "${BASE_DIR}/Scripts/BBOP_Pipeline/BBOP_step3_command_targeting_dynamic.sh" \
  "${ROIS[@]}"

# Step 4: Resample T1/T2 to 1mm isotropic
run_step "Step 4 resample-iso1mm" \
  "${BASE_DIR}/Scripts/BBOP_Pipeline/BBOP_step4_command_resample-isotropic.sh"

# Step 5: Coregister T2_iso1mm → T1_iso1mm (ANTS) + save FSL affine
run_step "Step 5 T1-T2-coreg" \
  "${BASE_DIR}/Scripts/BBOP_Pipeline/BBOP_step5_command_T1-T2_coreg_ANTS.sh"
